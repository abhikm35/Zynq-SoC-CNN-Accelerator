// cnn_accelerator_pingpong_top.sv
// Optimized end-to-end CNN top with two reusable ping-pong activation RAMs.
//
...
//   GAP:    read RAM A
//
// Arithmetic, weights, biases, and quantization are unchanged from
// cnn_accelerator_top. Stage controllers/engines are reused as-is.
// The original separate-memory top remains for regression.

`timescale 1ns / 1ps

module cnn_accelerator_pingpong_top #(
    ...
    // -------------------------------------------------------------------------
    // Conv1 engine reads activations
    logic eng1_act_re;
    logic [11:0] eng1_act_addr;
    logic signed [7:0] eng1_act_rdata;

    // Conv1 controller writes outputs
    logic conv1_we;
    ...
    // Conv2 engine reads activations
    logic eng2_act_re;
    logic [11:0] eng2_act_addr;
    logic signed [7:0] eng2_act_rdata;

    // Conv2 controller writes outputs
    logic conv2_we;
    ...
    // RAM A read: Conv1 | Conv2 | GAP | TB idle
    always_comb begin
        act_a_re    = 1'b0;
        act_a_raddr = 14'd0;
        if (own_conv1) begin
            act_a_re    = eng1_act_re;
            act_a_raddr = {2'b0, eng1_act_addr};
        end else if (own_conv2) begin
            act_a_re    = eng2_act_re;
            act_a_raddr = {2'b0, eng2_act_addr};
    ...
    always_comb begin
        ...
        if (own_conv1) begin
            act_b_we    = conv1_we;
            act_b_waddr = conv1_waddr;
            act_b_wdata = conv1_wdata;
        end else if (own_conv2) begin
            act_b_we    = conv2_we;
            act_b_waddr = {1'b0, conv2_waddr};
            act_b_wdata = conv2_wdata;
        end
    end
    ...
    assign eng1_act_rdata = act_a_rdata;
    assign eng2_act_rdata = act_a_rdata;
    ...
    conv1_memory_single_output #(
        ...
        .external_act_data(eng1_act_rdata),
    ...
    conv2_memory_single_output #(
        ...
        .external_act_data(eng2_act_rdata),
    ...
    always_ff @(posedge clk) begin
        if (!rst) begin
            ...
            if (own_conv1 && conv1_we && ({1'b0, conv1_waddr} >= 15'd16384))
                $error("Conv1 write OOB");
            if (own_pool1 && pool1_we && ({2'b0, pool1_waddr} >= 14'd4096))
                $error("Pool1 write OOB");
            if (own_conv2 && conv2_we && ({1'b0, conv2_waddr} >= 14'd8192))
            ...
            if (own_conv1 && eng1_act_re && ({2'b0, eng1_act_addr} >= 14'd3072))
                $error("Conv1 read OOB %0d", eng1_act_addr);
            if (own_pool1 && pool1_re && ({1'b0, pool1_raddr} >= 15'd16384))
                $error("Pool1 read OOB");
            if (own_conv2 && eng2_act_re && ({2'b0, eng2_act_addr} >= 14'd4096))
                $error("Conv2 read OOB %0d", eng2_act_addr);
    ...
    wire unused_dbg =
        |{eng1_final_acc, eng1_rq, eng1_ic, eng1_kr, eng1_kc, eng1_wgt_addr,
          ...
          gap_start, fc_start, argmax_start};
