// conv1_memory_single_output.sv
// Memory-driven single Conv1 output pixel.
//
// Given (output_channel, output_row, output_column):
//   1) Read bias / multiplier / shift from sync ROMs (1-cycle latency)
//   2) Load bias into INT32 accumulator
//   3) For ic=0..2, kr=0..2, kc=0..2 (kc fastest):
//        issue activation + weight addresses
//        wait one cycle for sync ROM data
//        force activation=0 on padding (no act ROM read issued when padded)
//        perform one MAC
//   4) Requantize + ReLU (reuse existing modules)
//
// FSM:
//   IDLE -> ISSUE_BIAS -> WAIT_BIAS -> LOAD_BIAS
//       -> ISSUE_OP -> WAIT_MAC  (x27)
//       -> REQUANTIZE -> DONE -> IDLE
//
// Timing (one MAC iteration):
//   Cycle N:   ISSUE_OP — addresses + read_enable presented
//   Cycle N+1: WAIT_MAC — ROM data valid; MAC enable asserted; counters advance
//
// Measured cycles from start-sampled to done pulse: 60 (see TB / docs).

`timescale 1ns / 1ps

module conv1_memory_single_output #(
    ...
    // When EXTERNAL_ACT=1, activation comes from shared input RAM (1-cycle latency).
    ...
    logic padding_w;
    logic signed [6:0] in_row_w;
    logic signed [6:0] in_col_w;
    logic [11:0] act_addr_w;
    logic [8:0]  wgt_addr_w;
    logic [3:0]  bias_addr_w;

    // Latched for the in-flight memory transaction (ISSUE -> WAIT)
    logic        padding_pend;
    logic [11:0] act_addr_pend;
    logic [8:0]  wgt_addr_pend;
    logic [1:0]  ic_pend;
    logic [1:0]  kr_pend;
    logic [1:0]  kc_pend;
    ...
    // When padding, activation forced to 0; weight always from ROM.
    assign act_mac = padding_pend ? 8'sd0 : act_rom_data;
    assign wgt_mac = wgt_rom_data;

    // Feed live ROM data into MAC during LOAD_BIAS so bias load and ROM
    // capture share the same posedge without an NBA race on bias_r.
    ...
    assign last_mac = (ic_r == 2'd2) && (kr_r == 2'd2) && (kc_r == 2'd2);
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                ST_ISSUE_BIAS: begin
                    // Addresses + read_enable presented; ROM data valid next cycle.
                end
                ST_WAIT_BIAS: begin
                    // ROM data becomes valid on the posedge entering this state;
                    // capture on the following edge in LOAD_BIAS (NBA race-safe).
                end
                ST_LOAD_BIAS: begin
                    // Capture sync-ROM outputs one full cycle after ISSUE_BIAS.
                ...
                ST_WAIT_MAC: begin
                    last_product <= prod_w;
                    macs_done_r  <= macs_done_r + 5'd1;
                    // Advance counters after MAC commit (kc fastest). Skip after last.
                    if (!last_mac) begin
                        if (kc_r == 2'd2) begin
                            kc_r <= 2'd0;
                            if (kr_r == 2'd2) begin
    ...
    always_comb begin
        ...
        case (state)
            ...
            ST_ISSUE_OP: begin
                // Never issue activation read for padded locations.
                act_re  = ~padding_w;
                wgt_re  = 1'b1;
                state_n = ST_WAIT_MAC;
            end
    ...
    always_ff @(posedge clk) begin
        if (!rst) begin
            if (mac_en && !load_bias_i) begin
                ...
                assert (macs_done_r < 5'd27)
                    else $error("mac_count would exceed 27");
            end
            if (done) begin
                assert (macs_done_r == 5'd27)
                    else $error("done with mac_count != 27");
            end
            if (act_re) begin
                assert (!padding_w)
                    else $error("activation read issued while padding");
                assert (act_addr_w < 12'd3072)
                    else $error("activation address out of range");
            end
            if (wgt_re) begin
                assert (wgt_addr_w < 9'd432)
                    else $error("weight address out of range");
            end
            if (state == ST_ISSUE_BIAS || state == ST_WAIT_BIAS || state == ST_LOAD_BIAS) begin
                assert (oc_r < 4'd16)
                    else $error("output_channel out of range");
            end
            if (state != ST_IDLE) begin
                assert (or_r < 5'd32 && oc_col_r < 5'd32)
                    else $error("output row/col out of range");
                assert (ic_r <= 2'd2 && kr_r <= 2'd2 && kc_r <= 2'd2)
                    else $error("kernel counters out of range");
            end
            if (state == ST_REQUANTIZE) begin
                assert (macs_done_r == 5'd27)
                    else $error("requantize before 27 MACs");
            end
        end
    end
