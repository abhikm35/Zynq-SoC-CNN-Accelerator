// cnn_accelerator_shared_compute_top.sv
// Ping-pong activation RAMs + shared Conv + shared MaxPool engines.
//
...
//
// Shared convolution: Conv1 then Conv2. Shared max-pool: Pool1 then Pool2.
// GAP/FC/Argmax unchanged. Prior shared-conv and ping-pong tops kept for regression.

`timescale 1ns / 1ps

module cnn_accelerator_shared_compute_top #(
    ...
    // Shared pool engine (Pool1 then Pool2): always read RAM B, write RAM A
    logic shared_pool_start;
    logic shared_pool_busy, shared_pool_done;
    logic shared_layer_is_pool2;
    logic shared_pool_re;
    logic [13:0] shared_pool_raddr;
    logic signed [7:0] shared_pool_rdata;
    logic shared_pool_we;
    logic [11:0] shared_pool_waddr;
    logic signed [7:0] shared_pool_wdata;
    logic [12:0] shared_pool_out_count;
    logic [14:0] shared_pool_read_count;
    logic shared_pool_dbg_layer;
    ...
    always_comb begin
        ...
        end else if (own_pool1 || own_pool2) begin
            act_a_we    = shared_pool_we;
            act_a_waddr = {2'b0, shared_pool_waddr};
            act_a_wdata = shared_pool_wdata;
        end
    end
    ...
    // RAM B read: Shared Pool | TB idle
    always_comb begin
        act_b_re    = 1'b0;
        act_b_raddr = 14'd0;
        if (own_pool1 || own_pool2) begin
            act_b_re    = shared_pool_re;
            act_b_raddr = shared_pool_raddr;
        end else if (~busy) begin
    ...
    assign shared_pool_rdata = act_b_rdata;
    ...
    // -------------------------------------------------------------------------
    // Shared MaxPool1 / MaxPool2 engine
    // -------------------------------------------------------------------------
    assign shared_pool_start     = pool1_start | pool2_start;
    assign shared_layer_is_pool2 = own_pool2;
    assign pool1_busy = own_pool1 & shared_pool_busy;
    assign pool2_busy = own_pool2 & shared_pool_busy;
    assign pool1_done = own_pool1 & shared_pool_done;
    assign pool2_done = own_pool2 & shared_pool_done;

    logic signed [7:0] sp_a, sp_b, sp_c, sp_d, sp_max;
    logic [4:0] sp_ch;
    logic [3:0] sp_pr, sp_pc;
    logic [1:0] sp_win;

    shared_maxpool_engine u_shared_pool (
        .clk(clk), .rst(rst),
        .start(shared_pool_start),
        .layer_is_pool2(shared_layer_is_pool2),
        .busy(shared_pool_busy),
        .done(shared_pool_done),
        .activation_read_enable(shared_pool_re),
        .activation_read_address(shared_pool_raddr),
        .activation_read_data(shared_pool_rdata),
        .output_write_enable(shared_pool_we),
        .output_write_address(shared_pool_waddr),
        .output_write_data(shared_pool_wdata),
        .current_channel(sp_ch),
        .current_pool_row(sp_pr),
        .current_pool_column(sp_pc),
        .current_window_index(sp_win),
        .output_count(shared_pool_out_count),
        .read_count(shared_pool_read_count),
        .dbg_layer_is_pool2(shared_pool_dbg_layer),
        .value_a(sp_a), .value_b(sp_b), .value_c(sp_c), .value_d(sp_d),
        .maximum_value(sp_max)
    );

    // Capture pool output counts at layer done
    logic [12:0] pool1_count_r;
    logic [11:0] pool2_count_r;
    always_ff @(posedge clk) begin
        if (rst) begin
            pool1_count_r <= 13'd0;
            pool2_count_r <= 12'd0;
        end else begin
            if (own_pool1 && shared_pool_done)
                pool1_count_r <= shared_pool_out_count;
            if (own_pool2 && shared_pool_done)
                pool2_count_r <= shared_pool_out_count[11:0];
        end
    end
    assign dbg_pool1_output_count = pool1_count_r;
    assign dbg_pool2_output_count = pool2_count_r;
    ...
    always_ff @(posedge clk) begin
        if (!rst) begin
            ...
            if (own_pool1 && shared_pool_we && ({2'b0, shared_pool_waddr} >= 14'd4096))
                $error("Pool1 write OOB");
            if (own_conv2 && shared_we && (shared_waddr >= 14'd8192))
                $error("Conv2 write OOB");
            if (own_pool2 && shared_pool_we && ({1'b0, shared_pool_waddr} >= 13'd2048))
            ...
            if (own_pool1 && shared_pool_re &&
                ({1'b0, shared_pool_raddr} >= 15'd16384))
                $error("Pool1 read OOB");
            if (own_conv2 && shared_act_re && ({2'b0, shared_act_addr} >= 14'd4096))
                $error("Conv2 read OOB %0d", shared_act_addr);
            if (own_pool2 && shared_pool_re && (shared_pool_raddr >= 14'd8192))
                $error("Pool2 read OOB");
            if (own_gap && gap_act_re && ({3'b0, gap_act_raddr} >= 14'd2048))
                $error("GAP read OOB");

            // Shared pool always reads B / writes A
            if ((own_pool1 || own_pool2) && shared_pool_we && act_b_we)
                $error("shared pool must not write RAM B");
            if ((own_pool1 || own_pool2) && shared_pool_re && act_a_re)
                $error("shared pool must not read RAM A");
            if (own_pool1 && own_pool2)
                $error("Pool1 and Pool2 ownership simultaneous");
    ...
    wire unused_dbg =
        |{sp_a, sp_b, sp_c, sp_d, sp_max, sp_ch, sp_pr, sp_pc, sp_win,
          shared_pool_read_count, shared_pool_dbg_layer,
