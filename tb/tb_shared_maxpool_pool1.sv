// tb_shared_maxpool_pool1.sv
// Shared max-pool engine in Pool1 mode vs Python golden (4096 outputs).

`timescale 1ns / 1ps

module tb_shared_maxpool_pool1 (
    input logic clk
);
    localparam int IN_N  = 16384;
    localparam int OUT_N = 4096;
    ...
    int8_sync_ram #(.DEPTH(IN_N), .ADDR_WIDTH(14)) u_in (
        .clk(clk),
        .write_enable(in_we),
    ...
    initial begin
        ...
        rst = 1; start = 0; layer_is_pool2 = 0;
        in_we = 0; in_waddr = 0; in_wdata = 0;
        for (i = 0; i < OUT_N; i = i + 1) seen[i] = 0;
        i = 0;
        $readmemh("vectors/end_to_end/conv1_expected.mem", exp_in);
        $readmemh("vectors/end_to_end/pool1_expected.mem", exp_out);
    end
    ...
    always @(posedge clk) begin
        ...
        if (phase == 2 && done) begin
            ...
            if (output_count !== 13'd4096) $fatal(1, "output_count %0d", output_count);
            if (read_count !== 15'd16384) $fatal(1, "read_count %0d", read_count);
            if (mismatches !== 0) $fatal(1, "mismatches=%0d", mismatches);
            if (dbg_layer !== 1'b0) $fatal(1, "layer flag");
            $display("PASS: tb_shared_maxpool_pool1 mismatches=0 writes=%0d reads=%0d",
                     wr_count, read_count);
            $finish;
        end
