// tb_shared_conv_engine_conv1.sv
// Shared convolution engine in Conv1 mode vs Python golden (16384 outputs).

`timescale 1ns / 1ps

module tb_shared_conv_engine_conv1 (
    input logic clk
);
    localparam int IN_N  = 3072;
    localparam int OUT_N = 16384;
    ...
    int8_sync_ram #(.DEPTH(3072), .ADDR_WIDTH(12)) u_in (
        .clk(clk),
        .write_enable(in_we),
    ...
    initial begin
        ...
        rst = 1; start = 0; layer_is_conv2 = 0;
        in_we = 0; in_waddr = 0; in_wdata = 0;
        for (i = 0; i < OUT_N; i = i + 1) seen[i] = 0;
        i = 0;
        $readmemh("vectors/end_to_end/input_image.mem", exp_in);
        $readmemh("vectors/end_to_end/conv1_expected.mem", exp_out);
    end
    ...
    always @(posedge clk) begin
        ...
        if (phase == 2 && done) begin
            ...
            if (output_count !== 15'd16384) $fatal(1, "output_count %0d", output_count);
            if (mismatches !== 0) $fatal(1, "mismatches=%0d", mismatches);
            $display("PASS: tb_shared_conv_engine_conv1 mismatches=0 writes=%0d", wr_count);
            $finish;
        end
